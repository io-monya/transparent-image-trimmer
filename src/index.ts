...
import { processImage, TrimResult } from './trimmer.js';
...
async function main() {
  ...
  p.intro(
    `${pc.bgCyan(pc.black(' 画像 余白トリミングツール '))} ${pc.dim('v1.0.0')}`
  );
  ...
  if (cliArgs.length > 0) {
    targetFiles = cliArgs;
    p.log.info(
      `コマンドライン引数から ${pc.cyan(targetFiles.length)} 件のファイルを受け取りました。`
    );
  } else {
    p.log.message(
      `macOSのファイル選択画面が開きます。\n${pc.dim(
        '※ Command(⌘)+クリック や Shift+矢印キー で複数ファイルを選択できます。'
      )}`
    );
  ...
  if (unsupportedFiles.length > 0) {
    p.log.warn(
      `以下のファイルは非対応形式のため除外されます (${unsupportedFiles.length}件):\n${unsupportedFiles
  ...
  p.log.success(
    `${pc.green(validFiles.length)} 件の画像ファイルが選択されました:\n${validFiles
      .slice(0, 5)
      .map((f) => `  ${pc.dim('•')} ${path.basename(f)}`)
  ...
  if (successList.length > 0) {
    p.log.success(
      `${pc.green(pc.bold(successList.length))} 件の画像をトリミングして保存しました:`
    );
    for (const item of successList) {
      ...
      console.log(
        `  ${pc.green('✔')} ${pc.bold(name)}: ${pc.dim(sizeChange)}  ${pc.cyan(`→ ${outName}`)}`
      );
    }
  }
  ...
  if (skippedList.length > 0) {
    console.log('');
    p.log.warn(
      `${pc.yellow(pc.bold(skippedList.length))} 件の画像をスキップしました:`
    );
    for (const item of skippedList) {
      const name = path.basename(item.inputPath);
      console.log(
        `  ${pc.yellow('⚠')} ${name}: ${pc.dim(item.reason || 'スキップされました')}`
      );
    }
  }
  ...
  if (errorList.length > 0) {
    console.log('');
    p.log.error(
      `${pc.red(pc.bold(errorList.length))} 件の処理でエラーが発生しました:`
    );
    for (const item of errorList) {
      const name = path.basename(item.inputPath);
      console.log(
        `  ${pc.red('✖')} ${name}: ${pc.dim(item.error || '不明なエラー')}`
      );
    }
  }
